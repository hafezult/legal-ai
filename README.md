...
- Matter conversations can be created or deleted with write permission; research queries also open a conversation thread automatically. Conversation create/message and matter create actions are rate-limited per user.
...
- Pending invite tokens and acceptance URLs are only loaded for owners and admins. Member roster emails are likewise admin/owner-only (members still see their own email).
...
- Public `/api/health` is a cheap process liveness probe (no database fan-out). Public `/api/ready` returns status + `checkedAt` only (HTTP 503 when critical probes are degraded) and rate-limits by client IP with a short in-process cache. Full probe details require `x-aether-health-secret` matching a strong `HEALTH_DETAIL_SECRET` or `INDEXING_SECRET` (placeholders such as `change-me` never unlock details). Settings shows live probes and env readiness to organization admins/owners only; aggregate readiness ignores optional OpenAI/indexing/Upstash configuration so missing AI keys do not mark the deployment unhealthy.
- Organization invite acceptance (token and auto email-match) claims invites with a conditional `acceptedAt` update inside a transaction so concurrent accepts cannot double-apply membership.
