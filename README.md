...
- Document upload posts to `/api/matters/[matterId]/documents`, which authenticates, rate-limits, and checks matter write access before parsing multipart bodies (Content-Length required; rejected above ~51 MB). Permission-check data-layer outages return HTTP 503 (not 403).
