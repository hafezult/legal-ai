...
- Public `/api/health` is a cheap process liveness probe (no database fan-out). `/api/ready` returns the full `getHealthReport()` payload with HTTP 503 when critical probes (database, Clerk, storage) are missing or degraded. Settings and the dashboard load the same dependency probes; aggregate readiness ignores optional OpenAI/indexing/Upstash configuration so missing AI keys do not mark the deployment unhealthy. Placeholder `INDEXING_SECRET` values are reported as degraded/not configured.
