...
- Each signed-in user receives a personal organization (owner role). First-provision races on the deterministic personal slug recover by re-querying the owner membership (P2002) instead of failing the shell as an empty workspace. Additional organizations can be created from Settings; matter creation uses the active organization.
