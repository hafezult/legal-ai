...
- List surfaces (dashboard, matters, documents, research, drafting, memory, workflows) scope to the active organization, plus legacy creator-owned matters with no organization attachment. Cross-matter registries and matter pickers use bounded `take` limits so large orgs cannot unbounded-load list queries.
